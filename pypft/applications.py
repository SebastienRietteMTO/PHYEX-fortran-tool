...
from util import (copy_doc, debugDecor,getIndexLoop,
                  alltext,tostring,getParent,getSiblings,moveInGrandParent)
from statements import (removeCall, setFalseIfStmt, createDoStmt,arrayRtoparensR,createArrayBounds,
                        createIfThenConstruct, createIfThenElseConstruct)
from variables import removeUnusedLocalVar,getVarList
...
def removeArraySyntax(doc,expandDoLoops=False,expandWhere=False):
    ...
    varArray = []
    varNameList = []
    Node_opE = doc.findall('.//{*}a-stmt')
    for var in varList:
        if var['as']:
            varArray.append(var)
            varNameList.append(var['n'])
    
    # Convert where-construct-stmt
    if expandWhere:
        whereConstruct = doc.findall('.//{*}where-construct')
        for node_where in whereConstruct:
            node_opE = node_where.findall('.//{*}where-construct-stmt/{*}*/{*}*')[0] #Last * is for finding op-E but also single array-R masks
            ...
            for node in arrayR:
                parensR=arrayRtoparensR(doc,node)
    ...
    for node_opE in Node_opE:
        nodePar = getParent(doc,node_opE)
        if not nodePar.tag.endswith('}where-block') and expandDoLoops:
            ...
            if not found_goldIndex and varName in varNameList:
                    ...
                    if alltext(sub) == ':': # ':' alone
                        ...
                        for j,el in enumerate(subPar):
                            if el == sub:
                                ...
                                break
                        # Get the variable object to find its declaration dimension
                        ind=varNameList.index(varName) #TODO varName is probably wrong here because multiple objects can be in E-2 and varName is from E-1
                        lowerBound = '1'
                        upperBound = str(varArray[ind]['as'][indextoTransform][1])
                        lowerXml,upperXml = createArrayBounds(lowerBound, upperBound)
                        sub.insert(0,lowerXml)
                ...
                for i,sub in enumerate(subsE1):
                    if alltext(sub) == ':': # ':' alone
                        ...
                        sub.insert(1,upperXml)
                        sub.text = '' # Delete the initial ':'
                        doStmt = createDoStmt(getIndexLoop(lowerBound, upperBound),lowerBound,upperBound)
                        doToBuild.append(doStmt)
                ...
                if len(doToBuild)>0:
                    ...
                    moveInGrandParent(doc,node_opE,nestedObj=doToBuild)

    # expandWhere if possible
    if expandWhere:
        for node_where in whereConstruct:
            nodePar = getParent(doc,node_where)
            ...
            moveInGrandParent(doc,node_opE,nestedObj=finalToBuild)
