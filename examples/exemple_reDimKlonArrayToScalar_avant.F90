...
REAL,        INTENT(IN)    :: PSIGQSAT(D%NIJT) ! use an extra "qsat" variance contribution (OSIGMAS case)
...
REAL,        OPTIONAL, INTENT(IN)    :: PICE_CLD_WGT(D%NIJT)
!
!
...
INTEGER   :: ITPL(D%NIJT)            ! top levels of troposphere
REAL      :: ZTMIN(D%NIJT)           ! minimum Temp. related to ITPL
!
...
REAL  :: ZPV(D%NIJT), ZPIV(D%NIJT), ZQSL(D%NIJT), ZQSI(D%NIJT) ! thermodynamics
REAL :: ZLL, DZZ, ZZZ                           ! used for length scales
REAL :: ZAH, ZDRW, ZDTL, ZSIG_CONV                     ! related to computation of Sig_s
REAL  :: ZA(D%NIJT), ZB(D%NIJT), ZSBAR(D%NIJT), ZSIGMA(D%NIJT), ZQ1(D%NIJT) ! related to computation of Sig_s
REAL  :: ZCOND(D%NIJT)
REAL  :: ZFRAC(D%NIJT)           ! Ice fraction
INTEGER  :: INQ1
REAL :: ZINC
...
REAL  :: ZDZ(D%NIJT), ZARDUM(D%NIJT), ZARDUM2(D%NIJT), ZCLDINI(D%NIJT)
! end OCND2
...
SELECT CASE(HFRAC_ICE)
  CASE ('T') !using Temperature
    ZFRAC(JIJ) = MAX( 0., MIN(1., (( NEBN%XTMAXMIX - PT(JIJ,JK) ) / ( NEBN%XTMAXMIX - NEBN%XTMINMIX )) ) ) ! freezing interval
  CASE ('O') !using Temperature with old formulae
    ZFRAC(JIJ) = MAX( 0., MIN(1., (( CST%XTT - PT(JIJ,JK) ) / 40.) ) ) ! freezing interval
  CASE ('N') !No ice
    ZFRAC(JIJ) = 0.
...
!error code IERR cannot be checked here to not break vectorization
  ...
  IF ( OSIGMAS ) THEN
    ...
    ! parameterize Sigma_s with first_order closure
      ...
      ZSIG_CONV =0.
      IF(LMFCONV) THEN
        ZSIG_CONV = ZCSIG_CONV * PMFCONV(JIJ,JK) / ZA(JIJ)
      END IF
