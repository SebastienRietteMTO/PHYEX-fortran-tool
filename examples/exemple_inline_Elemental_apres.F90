...
IF (GNEGT) THEN
 ZZW=ALOG(ZT(JL))
 ZUSW=EXP(CST%XALPW - CST%XBETAW/ZT(JL) - CST%XGAMW*ZZW)          ! es_w
 ZZW=EXP(CST%XALPI - CST%XBETAI/ZT(JL) - CST%XGAMI*ZZW)           ! es_i
END IF
  
ZSSI=0.
IF (GNEGT) THEN
  ZZW=MIN(PPRES(JL)/2., ZZW)             ! safety limitation
  ZSSI=ZVART(JL,IRV)*(PPRES(JL)-ZZW) / (CST%XEPSILO*ZZW) - 1.0
                                               ! Supersaturation over ice
  ZUSW=MIN(PPRES(JL)/2., ZUSW)            ! safety limitation
  ZUSW=(ZUSW/ZZW)*((PPRES(JL)-ZZW)/(PPRES(JL)-ZUSW)) - 1.0
...
IF (GNEGT) THEN
  PBU_INST(JL,IRVHENI_MR)=MAX(ZZW, 0.0)*ICEP%XMNU0/PRHODREF(JL)
...
ENDIF
IF (GNEGT) THEN
  PCIT(JL)=MAX(ZZW+PCIT(JL), PCIT(JL))
END IF
